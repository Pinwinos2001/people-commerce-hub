...
def render(usuario: str):
    ...
    st.markdown(
        '<div class="section-header">Pendientes</div>',
...
def _render_pendiente_card(row, usuario_actual):
    ...
    with col_s:
        nuevo_status = st.selectbox(
            "Cambiar estado",
            ESTADOS,
            index=ESTADOS.index(row.get("status", "Pendiente"))
            if row.get("status") in ESTADOS
            else 0,
    ...
    with st.form("nuevo_pendiente", clear_on_submit=True):
        ...
        with col1:
            ...
            responsable = st.selectbox(
                "Responsable *",
                USUARIOS,
                index=USUARIOS.index(usuario_actual)
        ...
        if submitted:
            ...
            if notificar and deadline:
                ok, msg = notif_nuevo_pendiente(
                    tema, responsable, str(deadline), EMAIL_CONFIG
                )
                ...
                if ok:
                    ...
                    st.warning(
                        f"No se pudo enviar la notificación: {msg}"
                    )
            else:
                st.success("Pendiente creado.")
